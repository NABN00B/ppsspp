...
void SavedataButton::UpdateText(const std::shared_ptr<GameInfo> &ginfo) {
	...
	if (!currentTitle.empty()) {
		title_ = SanitizeString(currentTitle, StringRestriction::NoLineBreaksOrSpecials, 0, 200);
	}
	if (subtitle_.empty() && ginfo->gameSizeOnDisk > 0) {
		...
		subtitle_ = SanitizeString(savedata_title, StringRestriction::NoLineBreaksOrSpecials, 0, 200) + " (" + NiceSizeFormat(ginfo->gameSizeOnDisk) + ", " + date + ")";
	}
}
